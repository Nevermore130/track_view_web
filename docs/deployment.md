...
- 每次构建写入独立的 `Git SHA + GitHub Run ID + Run Attempt` 目录，再原子切换
  `current`；同一提交重新构建也不会复用旧产物。
...
轻量应用服务器方案使用
`/srv/trace-atlas/releases/<git-sha>-<run-id>-<run-attempt>` 保存版本，并通过
`current` 符号链接原子切换。每个线上版本都能追溯到一个 Git commit 和一次具体
的 CI 构建；同一提交因环境变量变化而重新构建时也会产生独立版本。
